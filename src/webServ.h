...
void serverSetup()
{
    server.on("/test", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        {
            sendIt = request->getParam("t")->value();
            int mysendIt = sendIt.toInt();
            thisSend = String(mysendIt);
            Serial.println(mysendIt);
            Serial.println(thisSend);
        }
        request->send(200, "text/plain", sendIt + "  " + thisSend);
    });
    ...
    server.on("/alarm", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send_P(200, "text/html", alarmHome_html);
    });
    ...
    server.on("/time", HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send_P(200, "text/html", give_time);
    });
